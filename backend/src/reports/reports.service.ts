  ...
  return await new Promise((resolve, reject) => {
    try {
      ...
      const drawBrandedFirstPageHeader = () => {
        ...
        let yy = top + logoBox + 10;
        doc.font('Helvetica').fontSize(10).fillColor('#111827');
        doc.text(input.businessName, x, yy, { width: leftW, align: 'left' }); yy += 13;
        doc.font('Helvetica').fontSize(10).fillColor('#374151');
        doc.text(`Business ID: ${input.businessId}`, x, yy, { width: leftW, align: 'left' }); yy += 13;
        doc.text(`Branch ID: ${input.branchId}`, x, yy, { width: leftW, align: 'left' }); yy += 13;
        doc.text(`Business Type: ${input.businessType}`, x, yy, { width: leftW, align: 'left' }); yy += 13;
        const f = input.dateRange.from ? input.dateRange.from.toISOString().slice(0, 10) : '';
        const t = input.dateRange.to ? input.dateRange.to.toISOString().slice(0, 10) : '';
        doc.text(`Date Range: ${f} to ${t}`, x, yy, { width: leftW, align: 'left' });

        const dividerY = yy + 18;
        doc.moveTo(x, dividerY).lineTo(x + pageWidth, dividerY).lineWidth(1).strokeColor('#e5e7eb').stroke();
        doc.strokeColor('#000');
        doc.fillColor('#000');
