...
function applyPageFooter(doc: any) {
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    doc.text('Generated by SHUSHI HMS', x, footerY, { width: pageWidth / 2, align: 'left' });
...
function drawPnlFirstPage(
  ...
  // Business details (right aligned) - compact
  const rightW = 200;
  ...
  doc.text(`ID: ${input.businessId}  |  ${input.businessType}  |  Branch: ${input.branchId}`, rightX, top + 12, { width: rightW, align: 'right', lineBreak: false });
  doc.text(`Period: ${f} to ${t}`, rightX, top + 24, { width: rightW, align: 'right', lineBreak: false });
  doc.text(`Generated: ${formatDateTime(input.generatedAt)} by ${input.generatedBy}`, rightX, top + 36, { width: rightW, align: 'right', lineBreak: false });

  // Report title centered + divider
  const titleY = top + 58;
  doc.font('Helvetica-Bold').fontSize(14).fillColor('#111827');
  doc.text(input.title, x, titleY, { width: pageWidth, align: 'center', lineBreak: false });
  doc.moveTo(x, titleY + 20).lineTo(x + pageWidth, titleY + 20).lineWidth(0.5).strokeColor('#e5e7eb').stroke();
